...
const TTS_V3_URL = '/api/yandex/tts-v3';
const TTS_V1_URL = '/api/yandex/tts-v1';
const STT_V3_RECOGNIZE_URL = '/api/yandex/stt-recognize';
const STT_V3_RESULT_URL = '/api/yandex/stt-result';
...
function concatBase64Chunks(chunks: string[]) {
  const binary = chunks.map((chunk) => window.atob(chunk)).join('');
